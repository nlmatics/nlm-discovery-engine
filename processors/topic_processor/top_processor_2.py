...
class TopicProcessor(BaseProcessor):
    ...
    def cluster_(self, data, num_cluster=3, bic_=True, number_of_trials=10):
        bic = []
        if bic_:
            for k in range(1, len(data)):
                gm = GMM(n_components=k).fit(data)
                bic.append(gm.bic(data))

            n = np.argmin(bic) - 1
            num_cluster = n if n > 0 else 3

        models = []
        scores = []
        for i in range(number_of_trials):

            models.append(GMM(n_components=num_cluster).fit(data))
            scores.append(models[-1].score(data))
    ...
    def _cluster(self, phrases):
        ...
        clusters = self.cluster_(encs_proj)
